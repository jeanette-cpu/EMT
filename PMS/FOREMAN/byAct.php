...
<?php 
    ...
    if($v_cnt>0){
        while($row=mysqli_fetch_assoc($asgn_act_run)){
            ...
            // $bgt
            $q_bgt="SELECT Flat_Bgt_Manpower as bgt FROM flat_type_asgn_act as ft_act
                    LEFT JOIN flat_type as ft on ft.Flat_Type_Id = ft_act.Flat_Type_Id
                    LEFT JOIN flat_asgn_to_type as ft_flt on ft_flt.Flat_Type_Id=ft.Flat_Type_Id 
                    WHERE ft_flt.Flat_Id='$flat_id' AND ft.Flat_Type_Status=1 AND ft_act.Flt_Asgn_Act_Status=1 AND ft_flt.Flat_Asgd_Status=1 AND ft_act.Act_Id='$act_id'";
            $q_bgt_run=mysqli_query($connection,$q_bgt);
            if(mysqli_num_rows($q_bgt_run)>0){
                $row_b=mysqli_fetch_assoc($q_bgt_run);
                $bgt=$row_b['bgt'];
            }
            ?>
                <tr>
