    ...
    <div class="col-xl-12 col-lg-12">
        <div class="card-body">
            ...
            <table class="table table-bordered table-striped table-sm" id="dataTable" width="100%" cellspacing="0">
                ...
                <tbody>
    <?php
        $dept_q="SELECT * FROM department 
                WHERE `Dept_Name` LIKE '%Electrical%' OR `Dept_Name` LIKE '%Plumbing%' 
                OR `Dept_Name` LIKE '%HVAC%'";
        $dept_q_run=mysqli_query($connection,$dept_q);
        $total_cum=0; $ttotal_bgt=0;
        if(mysqli_num_rows($dept_q_run)>0){ $emt_all=0;$sb_all=0;$mp_all=0; $total_bgt=0; $rm_bgt_tot=0;
            while($row_d=mysqli_fetch_assoc($dept_q_run)){
                ...
                $total_per_dept=0;
                $total_per_villa=0;
                $q_types="SELECT Flat_Type_Id  FROM flat_type WHERE Flat_Type_Status=1 AND Prj_Id='$prj_id'";
                $q_types_run=mysqli_query($connection,$q_types);
                if(mysqli_num_rows($q_types_run)>0){
                    while($row_t=mysqli_fetch_assoc($q_types_run)){
                        $act_cnt=0;$flt_cnt=0;
                        $flt_type_id=$row_t['Flat_Type_Id'];
                        // count flats in flat type
                        $q_flt_asgn="SELECT * FROM flat_asgn_to_type WHERE Flat_Type_Id='$flt_type_id' AND Flat_Asgd_Status=1 ";
                        $q_flt_asgn_run=mysqli_query($connection,$q_flt_asgn);
                        $flt_cnt=mysqli_num_rows($q_flt_asgn_run);
                        // count bgt per activity
                        $q_flt_act="SELECT act.Act_Id, ft_asgn.Flat_Bgt_Manpower as bgt FROM flat_type_asgn_act as ft_asgn
                                    LEFT JOIN activity as act on act.Act_Id=ft_asgn.Act_Id
                                    WHERE ft_asgn.Flat_Type_Id='$flt_type_id' 
                                    AND ft_asgn.Flt_Asgn_Act_Status=1 AND act.Act_Status=1 AND act.Dept_Id='$dept_id' AND ft_asgn.Flat_Bgt_Manpower>0";
                        $q_flt_act_run=mysqli_query($connection,$q_flt_act);
                        if(mysqli_num_rows($q_flt_act_run)>0){
                            while($row_flat_asgn=mysqli_fetch_assoc($q_flt_act_run)){
                                $act_bgt=$row_flat_asgn['bgt'];
                                $act_cnt=$act_cnt+$act_bgt;
                                $act_arr[]=$row_flat_asgn['Act_Id'];
                            }
                        }
                        $comp_bgt_per_type=$flt_cnt*$act_cnt;
                        $total_per_dept=$total_per_dept+$comp_bgt_per_type;
                        // echo 'flat_count= '.$flt_cnt.' act budget'.$act_cnt.'<br>';
                    }
                    // echo $dept_name.' -'.$flt_cnt.' '.$act_cnt.'<br>';
                }
                $act_arr=array_unique($act_arr);
                $act_ids = implode("', '", $act_arr);
                $total_bgt=$total_bgt+$total_per_dept;
                // $total_per_dept $total_bgt $act_ids
                ...
                $dept_act="SELECT act.Act_Id, act.Act_Code, act.Act_Name, ft_act.Flat_Bgt_Manpower FROM flat_type AS ft 
                            ...
                            AND ft_act.Flt_Asgn_Act_Status=1 AND act.Act_Status=1 AND act.Dept_Id='$dept_id'";
            ...
            ?>
                <tr>
                    ...
                    <td class="text-right"><?php echo $tot_cum_bgt;?></td> <!--cummulative budget mp against current progress B-->
                    <td class="text-right"><?php echo abs($fvariance);?></td> <!--variance-->
                    <td class="text-right"><?php echo decFormat($pf);?></td>
                </tr>
            <?php
            ...
            $ttotal_bgt=$ttotal_bgt+ $total_per_dept;
             ...
             $mp_tot=0; $dept_total=0; $tot_cum_bgt=0; $total_bgt=0;
            }
        }
