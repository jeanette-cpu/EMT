...
if(isset($_POST['addDE']))
{
    ...
    if(mysqli_num_rows($c1_run)>0)
    {
        ...
        header('Location: activity_b.php?id='.$prj_id.'&flt_id='.$flt_id);
    }
    else
    {
        ...
        else
        {
            $_SESSION['status'] = "Error Adding Activity";
            $_SESSION['status_code'] = "error";
            header('Location: activity_b.php?id='.$prj_id.'&prj_name='.$prj_name);
        }
    }
}
// EDIT DAILY ACTIVITY
...
if(isset($_POST['search']))
{
    ...
    header('Location: activity_b.php?id='.$id.'&flt_id='.$flt_id);
}
