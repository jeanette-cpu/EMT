...
<div class="col-xl-12 col-lg-12">
    ...
    <table id="tbl1" class="table table-sm table-bordered"  style="width:100%;   border-collapse: collapse;">
        ...
        <tr class=" text-danger">
            <td class="text-center" colspan="6"><h5>EMT ELECTROMECHANICAL WORK LLC</h5></td> 
        </tr>
        <tr class=" text-danger">
            <td class="text-center" colspan="6"><h5>DAILY WORK REPORT</h5></td>
        </tr>
        <?php
        ...
        <tr>
            ...
            <!-- <td width="5%" class="text-center" style="color:black" >NO</td>
            <td width="40%" class="text-center" style="color:black">Description </td>
            ...
            <td class="text-center" style="color:black">Remarks</td>
                <?php
                    if(isset($flt_ids))
                    {
                        ...
                        AND daily_entry.User_Id='$user_id' and Flat_Id in ('$flt_ids')";
        ...
        ?>
                ...
                <?php
                    $q_elect_total = "SELECT COUNT(DISTINCT EMP.EMP_ID) as e_tot FROM daily_entry as DE LEFT JOIN asgn_worker as AS_W on AS_W.DE_Id = DE.DE_Id LEFT JOIN employee as EMP on EMP.EMP_ID = AS_W.Emp_Id LEFT JOIN assigned_activity as AS_ACT on AS_ACT.Asgd_Act_Id = DE.Asgd_Act_Id 
                    WHERE DE.DE_Status=1 AND DE.User_Id='$user_id' AND AS_W.Asgd_Worker_Status=1 AND AS_ACT.Asgd_Act_Status=1 AND EMP.EMP_STATUS=1 AND date(DE.DE_Date_Entry)='$date_today' AND AS_ACT.Flat_Id in ('$flt_ids') AND EMP.EMP_DESIGNATION LIKE '%elect%' ";
                    $q_elect_total_run = mysqli_query($connection,$q_elect_total);
                    $row_e_total = mysqli_fetch_assoc($q_elect_total_run);
                    // echo $q_elect_total;
                    // ELV elv_tot
                    $q_elv_total = "SELECT COUNT(DISTINCT EMP.EMP_ID) as elv_tot FROM daily_entry as DE LEFT JOIN asgn_worker as AS_W on AS_W.DE_Id = DE.DE_Id LEFT JOIN employee as EMP on EMP.EMP_ID = AS_W.Emp_Id LEFT JOIN assigned_activity as AS_ACT on AS_ACT.Asgd_Act_Id = DE.Asgd_Act_Id WHERE DE.DE_Status=1 AND DE.User_Id='$user_id' AND AS_W.Asgd_Worker_Status=1 AND AS_ACT.Asgd_Act_Status=1 AND EMP.EMP_STATUS=1 AND date(DE.DE_Date_Entry)='$date_today' AND AS_ACT.Flat_Id in ('$flt_ids') AND EMP.EMP_DESIGNATION LIKE '%ELV%'";
                    ...
                    where de.DE_Date_Entry='$date_today' and asmp.Asgn_MP_Status=1 
                    ...
                    LEFT JOIN daily_entry AS de on de.DE_Id=asmp.DE_Id 
                    where de.DE_Date_Entry='$date_today' and asmp.Asgn_SB_Status=1 
                    and de.DE_Status=1 and de.User_Id in ('$user_id')";
                   ...
                   //mp
                    $q_mp_elect="SELECT SUM(Asgn_MP_Total) as mp_e FROM asgn_mp LEFT JOIN daily_entry as de on de.DE_Id=asgn_mp.DE_Id
                                ...
                                WHERE de.DE_Date_Entry='$date_today' AND as_act.Act_Cat_Id IN ('1','2','3','4','5') and de.User_Id='$user_id' AND asgn_mp.Asgn_MP_Status=1 AND de.DE_Status=1";
                    ...
                    $q_sb_elect="SELECT SUM(Asgn_SB_Total) as sb_e FROM asgn_subcon 
                                ...
                                LEFT JOIN assigned_activity as as_act on as_act.Asgd_Act_Id=de.Asgd_Act_Id
                                WHERE de.DE_Date_Entry='$date_today' AND as_act.Act_Cat_Id IN ('1','2','3','4','5') and de.User_Id='$user_id' AND asgn_subcon.Asgn_SB_Status=1 AND de.DE_Status=1";
                    ...
                    $q_mp_plumb="SELECT SUM(Asgn_MP_Total) as mp_p FROM asgn_mp LEFT JOIN daily_entry as de on de.DE_Id=asgn_mp.DE_Id
                                LEFT JOIN assigned_activity as as_act on as_act.Asgd_Act_Id=de.Asgd_Act_Id
                                WHERE de.DE_Date_Entry='$date_today' AND as_act.Act_Cat_Id IN ('6','7','8','9','10') and de.User_Id='$user_id' AND asgn_mp.Asgn_MP_Status=1 AND de.DE_Status=1";
                    ...
                    $q_sb_plumb="SELECT SUM(Asgn_SB_Total) as sb_p FROM asgn_subcon 
                                LEFT JOIN daily_entry as de on de.DE_Id=asgn_subcon.DE_Id
                                LEFT JOIN assigned_activity as as_act on as_act.Asgd_Act_Id=de.Asgd_Act_Id
                                WHERE de.DE_Date_Entry='$date_today' AND as_act.Act_Cat_Id IN ('6','7','8','9','10') and de.User_Id='$user_id' AND asgn_subcon.Asgn_SB_Status=1 AND de.DE_Status=1";
                    ...
                    $q_mp_hvc="SELECT SUM(Asgn_MP_Total) as mp_hvc FROM asgn_mp LEFT JOIN daily_entry as de on de.DE_Id=asgn_mp.DE_Id
                                LEFT JOIN assigned_activity as as_act on as_act.Asgd_Act_Id=de.Asgd_Act_Id
                                WHERE de.DE_Date_Entry='$date_today' AND as_act.Act_Cat_Id IN ('11','12','13','14','15') and de.User_Id='$user_id' AND asgn_mp.Asgn_MP_Status=1 AND de.DE_Status=1";
                    ...
                    $q_sb_hvc="SELECT SUM(Asgn_SB_Total) as sb_hvc FROM asgn_subcon 
                                LEFT JOIN daily_entry as de on de.DE_Id=asgn_subcon.DE_Id
                                LEFT JOIN assigned_activity as as_act on as_act.Asgd_Act_Id=de.Asgd_Act_Id
                                WHERE de.DE_Date_Entry='$date_today' AND as_act.Act_Cat_Id IN ('11','12','13','14','15') and de.User_Id='$user_id' AND asgn_subcon.Asgn_SB_Status=1 AND de.DE_Status=1";
                ...
                <tr>
                    ...
                    <td class="text-right"><?php $hvac = $row_hvac_total1['hvac_tot1']+ $row_hvac_total2['hvac_tot2']; echo $hvac?></td>
                ...
                <tr>
                    ...
                    <td></td>
                    <td></td>
                    <td class="text-right"><?php echo $ff;?></td>
