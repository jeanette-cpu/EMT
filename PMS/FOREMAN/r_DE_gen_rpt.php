    ...
    <table id="tbl1" class="table table-sm table-bordered"  style="width:100%;  border-spacing: 0px;  border-collapse: collapse;">
                ...
                <?php
                    if(isset($flt_ids)){
                        $DE_q ="SELECT * FROM daily_entry 
                                ...
                                WHERE DE_Date_Entry BETWEEN '".$from."' AND '".$to."' AND DE_Status=1 
                                AND activity_category.Dept_Id='$dept_id' and Flat_Id in ('$flt_ids')
                                ORDER BY DE_Date_Entry";
        ...
        ?>
                ...
                <?php
                    ...
                    if($elec_uid_arr){
                        ...
                        $q_elect_total = "SELECT COUNT(DISTINCT EMP.EMP_ID) as e_tot FROM daily_entry as DE 
                                        ...
                                        WHERE DE.DE_Status=1 AND AS_W.Asgd_Worker_Status=1 AND EMP.EMP_STATUS=1 
                                        AND AS_ACT.Asgd_Act_Status=1 AND DE.DE_Date_Entry BETWEEN '".$from."' AND '".$to."' 
                                        AND AS_ACT.Flat_Id in ('$flt_ids') AND DE.User_Id in ('$elec_user_ids') GROUP BY DE.DE_Date_Entry";
                        $q_elect_total_run = mysqli_query($connection,$q_elect_total);
                        if(mysqli_num_rows($q_elect_total_run)>0){
                            while($row_e_total = mysqli_fetch_assoc($q_elect_total_run)){
                                $e_emt_tot=$e_emt_tot+$row_e_total['e_tot'];
                            }
                        }
                        //mp
                        $q_mp_elect="SELECT SUM(Asgn_MP_Total) as mp_e FROM asgn_mp 
                                    LEFT JOIN daily_entry as de on de.DE_Id=asgn_mp.DE_Id
                                    LEFT JOIN assigned_activity as as_act on as_act.Asgd_Act_Id=de.Asgd_Act_Id
                                    WHERE de.DE_Date_Entry BETWEEN '".$from."' AND '".$to."' AND as_act.Act_Cat_Id IN ('1','2','3','4','5') 
                                    and de.User_Id  in ('$elec_user_ids') AND asgn_mp.Asgn_MP_Status=1 AND de.DE_Status=1 AND as_act.Flat_Id in ('$flt_ids')";
                        ...
                        $q_sb_elect="SELECT SUM(Asgn_SB_Total) as sb_e FROM asgn_subcon 
                                    ...
                                    LEFT JOIN assigned_activity as as_act on as_act.Asgd_Act_Id=de.Asgd_Act_Id
                                    WHERE de.DE_Date_Entry BETWEEN '".$from."' AND '".$to."' AND as_act.Act_Cat_Id IN ('1','2','3','4','5') 
                                    and de.User_Id  in ('$elec_user_ids') AND asgn_subcon.Asgn_SB_Status=1 AND de.DE_Status=1  AND as_act.Flat_Id in ('$flt_ids')";
                        $q_sb_elect_run=mysqli_query($connection,$q_sb_elect); 
                        $row_sb_e=mysqli_fetch_assoc($q_sb_elect_run);$sb_e=$row_sb_e['sb_e'];
                    }
                    else{
                        $e_emt_tot=NULL; $mp_e=NULL; $sb_e=NULL;
                    }
                    if($plumb_uid_arr){ //PLUMBING
                        ...
                        $q_plumb_total = "SELECT COUNT(DISTINCT EMP.EMP_ID) as p_tot FROM daily_entry as DE 
                                    ...
                                    WHERE DE.DE_Status=1 AND AS_W.Asgd_Worker_Status=1 AND EMP.EMP_STATUS=1 
                                    AND AS_ACT.Asgd_Act_Status=1 AND DE.DE_Date_Entry BETWEEN '".$from."' AND '".$to."' 
                                    AND AS_ACT.Flat_Id in ('$flt_ids') AND DE.User_Id in ('$plumb_user_ids') GROUP BY DE.DE_Date_Entry";
                        $q_plumb_total_run = mysqli_query($connection,$q_plumb_total);
                        if(mysqli_num_rows($q_plumb_total_run)>0){
                            while($row_p_total = mysqli_fetch_assoc($q_plumb_total_run)){
                                $p=$p+$row_p_total['p_tot'];
                            }
                        }
                        //mp
                        $q_mp_plumb="SELECT SUM(Asgn_MP_Total) as mp_p FROM asgn_mp LEFT JOIN daily_entry as de on de.DE_Id=asgn_mp.DE_Id
                                LEFT JOIN assigned_activity as as_act on as_act.Asgd_Act_Id=de.Asgd_Act_Id
                                WHERE de.DE_Date_Entry BETWEEN '".$from."' AND '".$to."' AND as_act.Act_Cat_Id IN ('6','7','8','9','10') 
                                and de.User_Id in ('$plumb_user_ids') AND asgn_mp.Asgn_MP_Status=1 AND de.DE_Status=1  AND as_act.Flat_Id in ('$flt_ids')";
                        ...
                        $q_sb_plumb="SELECT SUM(Asgn_SB_Total) as sb_p FROM asgn_subcon 
                                    ...
                                    LEFT JOIN assigned_activity as as_act on as_act.Asgd_Act_Id=de.Asgd_Act_Id
                                    WHERE de.DE_Date_Entry BETWEEN '".$from."' AND '".$to."' AND as_act.Act_Cat_Id IN ('6','7','8','9','10') 
                                    and de.User_Id in ('$plumb_user_ids') AND asgn_subcon.Asgn_SB_Status=1 AND de.DE_Status=1  AND as_act.Flat_Id in ('$flt_ids')";
                        $q_sb_plumb_run=mysqli_query($connection,$q_sb_plumb);
                        $row_sb_p=mysqli_fetch_assoc($q_sb_plumb_run); $sb_p=$row_sb_p['sb_p'];
                    }
                    else{ $mp_p =NULL; $sb_p=null; $p=NULL;}
                    if($hvac_uid_arr){ // HVAC hvact_tot
                        $hvac_user_ids = implode("','", $hvac_uid_arr);
                        $q_hvac_total = "SELECT COUNT(DISTINCT EMP.EMP_ID) as hvac_tot FROM daily_entry as DE 
                                    LEFT JOIN asgn_worker as AS_W on AS_W.DE_Id = DE.DE_Id 
                                    LEFT JOIN employee as EMP on EMP.EMP_ID = AS_W.Emp_Id 
                                    LEFT JOIN assigned_activity as AS_ACT on AS_ACT.Asgd_Act_Id = DE.Asgd_Act_Id 
                                    WHERE DE.DE_Status=1 AND AS_W.Asgd_Worker_Status=1 AND AS_ACT.Asgd_Act_Status=1 
                                    AND EMP.EMP_STATUS=1 AND DE.DE_Date_Entry BETWEEN '".$from."' AND '".$to."'  
                                    AND AS_ACT.Flat_Id in ('$flt_ids') AND DE.User_Id in ('$hvac_user_ids') GROUP BY DE.DE_Date_Entry";
                        $q_hvac_total_run = mysqli_query($connection,$q_hvac_total);
                        if(mysqli_num_rows($q_hvac_total_run)>0){
                            while($row_hvac_total = mysqli_fetch_assoc($q_hvac_total_run)){
                                $hvac=$hvac+$row_hvac_total['hvac_tot'];
                            }
                        }
                        else{ $hvac=NULL;}
                        //mp
                        $q_mp_hvc="SELECT SUM(Asgn_MP_Total) as mp_hvc FROM asgn_mp LEFT JOIN daily_entry as de on de.DE_Id=asgn_mp.DE_Id
                                    LEFT JOIN assigned_activity as as_act on as_act.Asgd_Act_Id=de.Asgd_Act_Id
                                    WHERE de.DE_Date_Entry BETWEEN '".$from."' AND '".$to."' AND as_act.Act_Cat_Id IN ('11','12','13','14','15') 
                                    and de.User_Id in ('$hvac_user_ids') AND asgn_mp.Asgn_MP_Status=1 AND de.DE_Status=1  AND as_act.Flat_Id in ('$flt_ids')";
                        ...
                        $q_sb_hvc="SELECT SUM(Asgn_SB_Total) as sb_hvc FROM asgn_subcon 
                                    ...
                                    LEFT JOIN assigned_activity as as_act on as_act.Asgd_Act_Id=de.Asgd_Act_Id
                                    WHERE de.DE_Date_Entry BETWEEN '".$from."' AND '".$to."' AND as_act.Act_Cat_Id IN ('11','12','13','14','15') 
                                    and de.User_Id in ('$hvac_user_ids') AND asgn_subcon.Asgn_SB_Status=1 AND de.DE_Status=1  AND as_act.Flat_Id in ('$flt_ids')";
                        $q_sb_hvc_run=mysqli_query($connection,$q_sb_hvc);
                        $row_sb_hvc=mysqli_fetch_assoc($q_sb_hvc_run); $sb_hvc=$row_sb_hvc['sb_hvc'];
                    }
                    else{
                        $hvac=NULL; $mp_hvc=NULL; $sb_hvc=NULL;
                    }
                ?>
                <tr>
                    ...
                    <td class="text-right"><?php echo $e_emt_tot?></td>
                    <td class="text-center"><?php echo $elect_t= $e_emt_tot+$mp_e+$sb_e;?></td>
                </tr>
                <tr>
                    <td colspan="2">PLUMBING</td>
                    <td class="text-right"><?php echo $mp_p?></td>
                    <td class="text-right"><?php echo $sb_p?></td>
                    <td class="text-right" ><?php  echo $p ?></td>
                ...
                <tr>
                    ...
                    <td class="text-right"><?php $d_total = $e_emt_tot+$hvac+$p;echo $d_total?></td>
