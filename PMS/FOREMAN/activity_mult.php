...
if(isset($_POST['actBtn']) or isset($_GET['id']))
{
    if(isset($_POST['actBtn'])){
        ...
        if(mysqli_num_rows($run_q)>0){
            $flat_name='<span  class=" pr-3 font-weight-bold">Updating on: </span><br>';
            $flats=''; $f_count=1;
            while($row4 = mysqli_fetch_assoc($run_q)){
                // $lvl_name= 'Level - '.$row4['Lvl_Code'].' '.$row4['Lvl_Name'].'<br>';
                $flats .= '<span class="pl-3">'.$f_count.'. '.$row4['flat_code'].' '.$row4['flat_name'].'</span><br>';
                $f_count++;
            }
        }
        $flat_name.=$flats;
...
<div class="col-xl-12 col-lg-12">
        ...
        <form action="code_mult.php" method="POST">
            <div id="villa">
                <div class="row pb-3 pl-3 pt-2">
                    ...
                    <div class="col-2">
                        <label>Villa</label>
                        <select name="flt_id[]" id="flt_opt" class="form-control selectpicker" data-live-search="true" multiple required>
                            <option value="">Select Plex</option>
                        </select>
                    </div>
                    <div class="col-2">
            ...
            <div class="row pb-3 pl-3 pt-2">
                <div class="col-4">
                    <form action="code_mult.php" method="post">
                        <label for="">All Villa</label>
                        <select name="flt_id[]" id="" class="form-control selectpicker" data-live-search="true" multiple required>
                            <option value="">Select Flat</option>
                            <?php 
                                $q_flats="SELECT * FROM flat as f 
                                            LEFT JOIN level AS l on l.Lvl_Id=f.Lvl_Id 
                                            LEFT JOIN building as b on b.Blg_Id=l.Blg_Id
                                            LEFT JOIN plex AS plx on plx.Plx_Id=b.Plx_Id
                                            WHERE f.Flat_Id IN ('$all_flt_ids') AND l.Lvl_Status=1 AND b.Blg_Status=1";
                                ...
                                if(mysqli_num_rows($q_flats_run)>0){
                                    while($row_f=mysqli_fetch_assoc($q_flats_run)){
                                        ...
                                        $Plx_code=$row_f['Plx_Code'];
                                        ?>
                                        <option value="<?php echo $flat_id;?>"><?php echo $Plx_code.' '.$flt_name_opt;?></option>
                                        <?php
                                    }
                                }
                            ?>
    ...
    <div class="card shadow mb-4 pb-3 pl-4 pr-4 pt-3" id="section_2">
    ...
    if(mysqli_num_rows($q_manpower_run)>0){
        ...
        <form method="POST" action="code_mult.php">
            <div class="row">
                <div class="col-12">
        ...
        // subcontractor
        $q_subcon ="SELECT * FROM daily_entry as de 
                    LEFT JOIN asgn_subcon as assb on de.DE_Id = assb.DE_Id 
        ...
        if(mysqli_num_rows($q_subcon_run)>0){
            ...
            $sb_table .="
                ...
                <div class='col-7'>
                    <button name='sb_total' class='btn btn-success float-right' type='submit'>Save</button>
                </div>
            </form>";
            echo $sb_table;
        }
...
<?php
    ...
    if(mysqli_num_rows($query_run2)>0){
        ...
        $query_run = mysqli_query($connection, $query); 
        // $query_run_opt =mysqli_query($connection,$query);
    }
?>
<div>
    <div class="col-xl-12 col-lg-12">
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body" >
        <!-- THE FORM -->
        <form action="code_mult.php" method="POST" id="addmodal">
