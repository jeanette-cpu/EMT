...
if(isset($_POST['addDE'])){
    ...
    if(mysqli_num_rows($q_act_ids_run)>0){
        while($row_act=mysqli_fetch_assoc($q_act_ids_run)){
            ...
            // $asgn_no =mysqli_num_rows($query_run1);
                            ...
                            if(isset($_POST['employee'])){ 
                                ...
                                if($e_mp!=NULL AND $e_mp!='' AND $e_mp!='Select Employee'){
                                    ...
                                    for ($i=0; $i < $count; $i++) {
                                        ...
                                        $emp_run=mysqli_query($connection,$query1);
                                        if($emp_run){
                                        }
                                        else{
    ...
    header('Location: activity_b_mult.php?id='.$id.'&flt_id='.$flt_id.'&date='.$Date);
}
// EDIT DAILY ACTIVITY
...
if(isset($_POST['delEmp'])){
    $ids = $_POST['id']; //assigned worker id
    $id = $_POST['prj_id'];
    $flt_id = $_POST['flt_id'];
    $Date =$_POST['date'];
...
if(isset($_POST['delMP'])){
    ...
    if($mp_row['t'] !=NULL){
        ...
        $Date = $_POST['date'];
        $mp_id = $mp_row['MP_Id'];
        $c_mp="SELECT Asgn_MP_Id FROM asgn_mp as asmp LEFT JOIN daily_entry AS de on de.DE_Id=asmp.DE_Id where de.DE_Date_Entry='$Date' AND asmp.MP_Id='$mp_id' and asmp.Asgn_MP_Status=1 and de.DE_Status=1  and asmp.Asgn_MP_Total is NULL LIMIT 1";
        $c_mp_run = mysqli_query($connection,$c_mp);
        $row_mp = mysqli_fetch_assoc($c_mp_run);
        if(mysqli_num_rows($c_mp_run)>0){
            //UPDATE THE mp total value
