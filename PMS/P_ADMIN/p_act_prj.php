...
<!-- ACTIVITY -->
    <div class="card shadow mb-4 col-md-11">
        <div class="card-header py-3">
            <h5 class="font-weight-bold text-primary"><i class="fa fa-cube mr-2" aria-hidden="true"></i>Manage Activity</h5>
        </div>
        <label>Project</label>
            <select name="prj_id" id="act_project" class="mt-2 mb-3 form-control selectpicker lvl_project" data-live-search="true" required>
                <option value="">Select Project</option>
            </select>
         <!-- Building Type -->
         <div id="blg_div" class="mt-3-2 mb-3 d-none">
            <div class="row">
                <div class="col-6">
            ...
            <form action="p_asgn_activity.php" method="POST">
                    <label class="mt-2">Flat</label>
                    <select name="flat_id" id="flat_name" class="form-control selectpicker" data-live-search="true" required>
                        <option value="">Select Flat</option>
                    </select>
                <input type="hidden" name="prj_name" id="flt_bprj_name">
                <div class="pull-right mt-3">
                    <button type="submit" name="actBtn" class="btn btn-success"> Search</button>
                </div>
            </form>
        </div>
        <!-- Villa Type -->
        <div id="villa_div" class="d-none">
            ...
            <form action="p_asgn_activity.php" method="POST">
                ...
                <select name="flat_id" id="v_flat" class="form-control selectpicker" data-live-search="true" required>
                    <option value="">Select Flat</option>
                </select>
...
$.ajax({
    ...
    success:function(data){
        $('#act_project').html(data).change();
        $('.selectpicker').selectpicker('refresh');
    }
});
var ctr=0;
    $('#act_project').on('change', function (){
...
var fblg_id = $(this).val();
    ...
    $.ajax({
        ...
        data:{'lvl_id': fblg_id},
        success:function(data){
            $('#flat_name').html(data).change();
            $('.selectpicker').selectpicker('refresh');
        }
    });
});
//for prj name - building type
...
var fblg_id = $(this).val();
    ...
    $.ajax({
        ...
        data:{'lvl_id': fblg_id},
        success:function(data){
            $('#v_flat').html(data).change();
            $('.selectpicker').selectpicker('refresh');
        }
    });
});
//for prj name - villa type
$('#v_flat').on('change', function(){
    var flat_id = $(this).val();
        $.ajax({
            url: 'ajax_villa.php',
            method: 'POST',
